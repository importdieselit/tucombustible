...
class CreateInspeccionImagenesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('inspeccion_imagenes', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('inspeccion_id')->index();
            $table->string('ruta_imagen', 500)->comment('Ruta de la imagen en el servidor');
            $table->string('descripcion', 255)->nullable()->comment('Descripción opcional de la evidencia');
        ...
        // Agregar la foreign key en una instrucción separada
        Schema::table('inspeccion_imagenes', function (Blueprint $table) {
