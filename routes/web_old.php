    ...
    Route::patch('/usuarios/{id}/toggle-estatus', [UserController::class, 'toggleEstatus'])->name('usuarios.toggle-estatus');
     ...
     // Órdenes (OT)
       ...
       Route::prefix('rrhh')->name('rrhh.')->middleware(['auth'])->group(function () {
        ...
        Route::prefix('admin/parametros')->name('parametros.')->middleware(['auth'])->group(function () {
    
        // ---------------------------------------------------------
        // 1. RUTAS DE USUARIO (Llenar los valores - Vista Pestañas)
        // ---------------------------------------------------------
        Route::get('/', [ParametroController::class, 'index'])->name('index');
        Route::put('/update', [ParametroController::class, 'update'])->name('update');

        // ---------------------------------------------------------
        // 2. RUTAS SUPERADMIN (Schema Builder - Crear reglas)
        // ---------------------------------------------------------
        Route::prefix('manager')->name('manager.')->group(function () {
            ...
            Route::get('/{parametro}/edit', [ParametroManagerController::class, 'edit'])->name('edit');
            Route::put('/{parametro}', [ParametroManagerController::class, 'update'])->name('update');
            Route::delete('/{parametro}', [ParametroManagerController::class, 'destroy'])->name('destroy');
            
            // API Interna para JS (Obtener columnas de BD)
